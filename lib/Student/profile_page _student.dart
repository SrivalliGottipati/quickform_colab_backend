...
class _StudentProfilePageState extends State<StudentProfilePage> {
  ...
  final TextEditingController sectionController = TextEditingController();
  String selectedThubStatus = 'Thub';
  ...
  Future<void> loadStudentData() async {
    ...
    setState(() {
      studentData = doc.data();
      sectionController.text = studentData?['section'] ?? '';
      selectedThubStatus = studentData?['thubStatus'] ?? 'Thub';
      isLoading = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      extendBodyBehindAppBar: true, // make AppBar float over body
      appBar: AppBar(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildProfileItem(Icons.email, 'Email', studentData?['email'] ?? 'N/A'),
                    _buildProfileItem(Icons.calendar_today, 'Year', studentData?['year'].toString() ?? 'N/A'),
                    _buildProfileItem(Icons.code, 'Branch', studentData?['branch'] ?? 'N/A'),
                    _buildProfileItem(Icons.school, 'Course', studentData?['course'] ?? 'N/A'),
                    _buildProfileItem(Icons.location_city, 'College', studentData?['college'] ?? 'N/A'),
  ...
  Widget _buildThubDropdown() {
    return Row(
      children: [
        const Icon(Icons.verified_user, color: Colors.lightBlue),
        const SizedBox(width: 12),
        const Text('T-Hub Status:', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(width: 8),
        Expanded(
          child: DropdownButtonFormField<String>(
            value: selectedThubStatus,
            items: thubOptions.map((status) {
